...
async def _translate_json_data(
    ...
    from core.json_utils import extract_text_blocks, rebuild_json_with_translations, extract_table_cells, rebuild_table_html_with_translations

    # 创建统一的 semaphore（仅在非本地模式下启用并发限制）
    semaphore = asyncio.Semaphore(concurrency) if base_url != "local" else None

    # =============== 第一步：翻译普通文本块（content 字段） ===============
    text_items = extract_text_blocks(json_data)
    translated_data = json_data  # 初始为原数据
    ...
    if text_items:
        ...
        if base_url == "local":
            ...
            tasks = [
                _translate_group_with_semaphore(
                    ...
                    semaphore=semaphore,  # 使用统一 semaphore
                )
        ...
        translated_data = [{**item} for item in json_data]  # 浅拷贝，为后续表格修改做准备

    # =============== 第二步：翻译表格（type == "table"） ===============
    table_entries = extract_table_cells(translated_data)
    if table_entries:
        logger.info(f"Found {len(table_entries)} tables to translate in {file_path}")

        async def _translate_single_table(table_index: int, cell_texts: List[str]):
            if base_url == "local":
                # 本地模式：不限流（或你可自行加 semaphore）
                try:
                    result = await translate_text_list_locally(
                        cell_texts, target_lang, model_path=model_name
                    )
                    return table_index, result
                except Exception as e:
                    logger.warning(f"Local table translation failed at index {table_index}: {e}")
                    return table_index, None
            else:
                # API 模式：受统一 semaphore 限制
                async with semaphore:  # ← 关键：复用同一个并发控制
                    try:
                        result = await translate_text_list_as_json(
                            texts=cell_texts,
                            target_lang=target_lang,
                        ...
                        if len(result) != len(cell_texts):
                            logger.warning(f"Table {table_index}: translation length mismatch ({len(result)} vs {len(cell_texts)})")
                            return table_index, None
                        return table_index, result
                    except Exception as e:
                        logger.warning(f"Failed to translate table {table_index} in {file_path}: {e}")
                        return table_index, None

        # 构建所有表格翻译任务（跳过空表格）
        table_tasks = [
            _translate_single_table(idx, cell_texts)
            for idx, cell_texts in table_entries
            if cell_texts
        ]
        ...
        if table_tasks:
            ...
            for res in table_results:
                if isinstance(res, Exception):
                    logger.error(f"Table task raised exception: {res}")
                    continue
                table_idx, translated_cells = res
                if translated_cells is not None:
                    try:
                        original_html = translated_data[table_idx]["html"]
                        new_html = rebuild_table_html_with_translations(original_html, translated_cells)
                        translated_data[table_idx]["html"] = new_html
                    except Exception as e:
                        logger.error(f"Error rebuilding HTML for table {table_idx}: {e}")
